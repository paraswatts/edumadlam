...
const CustomMCQModal = ({
    ...
    const renderOption = (buttonLabel, buttonOption, index, buttonStyles, _id) => {
        ...
        let isSelected = answersList && answersList.length && answersList.findIndex((answerObj) => answerObj._id === _id)
        //option === buttonOption 
        return (<CustomButton
            left={<MaterialCommunityIcons name={isSelected >= 0 && option === buttonOption ? "checkbox-marked-circle" : "checkbox-blank-circle"}
                color={isSelected > 0 && option === buttonOption ? COLORS.GREEN : '#c2c2c2'} size={20} />}
