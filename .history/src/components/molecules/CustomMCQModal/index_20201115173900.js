...
const CustomMCQModal = ({
    ...
    const onPressOption = (option, questionIndex, _id) => {
        ...
        let optionIndex = answersList && answersList.length && answersList.findIndex((answerObj) => answerObj._id === _id)
        if (optionIndex === -1) {
            updateAnswersList(answersList => [...answersList, { _id: _id, selectedOption: option }])
        }
        else {
            updateAnswersList(objs.map((o) => {
                if (o._id === _id) return { ...o, selectedOption: option }
                return o;
            }));
        }
