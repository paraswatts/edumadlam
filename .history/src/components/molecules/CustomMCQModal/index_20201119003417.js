...
const CustomMCQModal = ({
    ...
    const onPressOption = (option, questionIndex, _id) => {
        if (questionIndex === questionsObj.length - 1 && questionsObj.length > 1) {
            ...
            let optionIndex = answersList.findIndex((answerObj) => answerObj._id === _id)
            console.log(answersList, "optionIndex", optionIndex)
            if (optionIndex === -1) {
                updateAnswersList(answersList => [...answersList, { _id: _id, answer: option }])
            }
            else {
                updateAnswersList(answersList.map((o) => {
                    if (o._id === _id) return { ...o, answer: option }
                    return o;
                }));
            }
