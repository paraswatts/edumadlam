...
const CustomModal = ({
    ...
    const renderQuestionitem = ({ item, index }) => {
        const { qId, _quest, answer, status, remark, grade } = item
        let icon, color
        if (status == 0) {
            ...
            >
                ...
                <View style={{ flex: 1 }}>
                    <View style={{ alignItems: 'center' }}>
                        ...
                        <View style={{ flexDirection: 'row', borderTopWidth: 0.5, width: '100%', padding: _scaleText(10).fontSize }}>
                            {!resultObj.pdf ? <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, justifyContent: 'center' }}><MaterialCommunityIcons name={'close-circle-outline'}
