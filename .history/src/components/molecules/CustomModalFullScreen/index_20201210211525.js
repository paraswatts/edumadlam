...
const CustomModal = ({
    ...
    const renderQuestionitem = ({ item, index }) => {
        ...
        return (
            <View style={{ flexDirection: 'row', marginVertical: _scaleText(5).fontSize, marginHorizontal: _scaleText(10).fontSize borderWidth: _scaleText(0.5).fontSize, padding: _scaleText(5).fontSize, borderRadius: _scaleText(5).fontSize }}>
                <MaterialCommunityIcons name={answer ? "checkbox-marked-circle" : "checkbox-blank-circle"}
                    color={answer ? COLORS.GREEN : '#c2c2c2'} size={20} />
                <Text style={{ marginLeft: _scaleText(5).fontSize }}>{'Q'}{ }</Text>{<HTMLView value={_quest.replace(/(\r\n|\n|\r)/gm, "")} />}
            </View>
        )
    }
    return (
