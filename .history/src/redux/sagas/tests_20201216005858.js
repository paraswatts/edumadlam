...
function* generatePaymentLink({ payload: { netConnected, json, success = () => { }, fail = () => { } } = {} }) {
    try {
        console.log("answersList", json)
        let jsonPayload = JSON.stringify(json)
        if (netConnected) {
            console.log(API.GENERATE_PAYMENT_LINK(`?json=${jsonPayload}`))
            yield put(startLoading());
            const { data = {}, status } = yield postRequestWithParams({
                API: API.GENERATE_PAYMENT_LINK(`?json=${jsonPayload}`)
            })
