...
function* generatePaymentLink({ payload: { netConnected, amount, purpose, sId, type, productId, success = () => { }, fail = () => { } } = {} }) {
    try {
        ...
        if (netConnected) {
            console.log(API.GENERATE_PAYMENT_LINK(encodeURIComponent`?amount=${amount}&purpose=${encodeURIComponent(purpose)}&sId=${sId}&type=${type}&productId=${productId}`))
            yield put(startLoading());
            const { data = {}, status } = yield getRequest({
                API: API.GENERATE_PAYMENT_LINK(encodeURIComponent`?amount=${amount}&purpose=${encodeURIComponent(purpose)}&sId=${sId}&type=${type}&productId=${productId}`)
            })
