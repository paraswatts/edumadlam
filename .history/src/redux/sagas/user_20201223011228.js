...
function* getUserPurchaseHistorySaga({ payload: { netConnected, payload = {}, success = () => { }, fail = () => { } } = {} }) {
    try {
        if (netConnected) {
            yield put(startLoading());
            const { status, data = {} } = yield postRequest({
                API: API.GET_USER_PURCHASE_HISTORY(`?sId=${payload.sId}`)
            })
            console.log(data, "profile data");
            if (status == 200) {
                success(data)
...
function* AuthSaga() {
    yield all([
        ...
        takeLatest(GET_USER_PURCHASE_HISTORY_REQUEST, getUserPurchaseHistorySaga)])
