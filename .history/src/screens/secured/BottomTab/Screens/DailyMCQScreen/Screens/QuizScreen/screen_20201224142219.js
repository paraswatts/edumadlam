...
const QuizScreen = ({
    ...
    console.log(moment(selectedDate).format("DDth MMM yyyy"))
    return (
        <ScreenHOC
        ...
        >
            ...
            { loading && !exiting ? <ActivityIndicator size={'large'} color={COLORS.GREY._2} />
                :
                data && data.length ?
                    <CustomMCQModal date={moment(selectedDate).format("DD-MM-yyyy")} name={"MCQ"} ref={childRef} submitTest={submitTest} updateAnswerList={updateAnswerList} testStarted={testStarted} resetTimer={resetTimer} questionsObj={data} />
