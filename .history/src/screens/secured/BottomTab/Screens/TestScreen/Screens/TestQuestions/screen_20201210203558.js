...
const TestQuestions = ({
    ...
    const mergeArraysAnswers = (arr1, arr2) => {
        for (let x in arr1) {
            ...
            var filteredElements = arr2.filter(function (item, index) { return item.qId == obj._id; });
            if (!filteredElements.length) {
                arr2.push({ qId: obj._id, answer: false, _quest: _quest })
            }
            // console.log(arr2)
        }
        return arr2
    ...
    const updateAnswerList = (answersList, answersListObj) => {
        console.log(answersListObj, "answersList", answersList)
        let answersObj = mergeArrays(data, answersListObj)
        updateAnswersListObj(answersListObj)
