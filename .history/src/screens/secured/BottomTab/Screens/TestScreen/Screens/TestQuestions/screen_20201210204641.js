...
const TestQuestions = ({
    ...
    useEffect(() => {
        let array = []
        data.map((obj) => array.push({ qId: obj._id, answer: false, _quest: obj._quest }))
    ...
    return (
        ...
        >
            ...
            { loading && !exiting ? <ActivityIndicator size={'large'} color={COLORS.GREY._2} /> :
                <CustomMCQModal _testDuration={_testDuration} submitTest={submitTest} answersListObj={answersListObj} updateAnswerList={updateAnswerList} testStarted={testStarted} questionsObj={data} />}
        </ScreenHOC>
    );
}
