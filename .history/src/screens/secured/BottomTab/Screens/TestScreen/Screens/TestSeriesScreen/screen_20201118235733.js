            ...
            <FlatList
                ...
                renderItem={({ item, index }) => {
                    ...
                    return (<TouchableOpacity onPress={() => navigation.navigate(ROUTES.TEST.LIST, { _id: _id, _category: _category })}
                        ...
                        <Text>{_category}</Text>
                        <Text style={{ fontWeight: '500', fontSize: 14, marginTop: 10 }}><Text style={styles.fontBold}>Price: </Text>{`₹${_price}`}</Text>
                        <TouchableOpacity style={{ flexDirection: 'row', alignSelf: 'flex-end', alignItems: 'center' }}>
                            <Text style={[styles.fontBlue, { textAlign: 'right', marginRight: _scaleText(5).fontSize }]}>{TEXT_CONST.PURCHASE_TEST_SERIES}</Text>
